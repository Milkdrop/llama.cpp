...
//static_assert(sizeof(block_q4_x) == sizeof(ggml_fp16_t) + QK4_X * 2, "wrong q4_x block size/padding");
        ...
        for (int jb = 0; jb < QK4_X / 64; jb++) {
            ...
            if (fp16_chooser == 0 && false) {
                if (data_offset == 0) {
                    ...
                    const uint8_t num_of_q8_blocks = 64 / QK8_0;
                    const uint8_t num_of_data_blocks_per_q8 = QK8_0 / weights_per_u64_data_block; // because we have 64 qbit-sized weights here 
                    
                    for (int q8_pair = 0; q8_pair < num_of_q8_blocks; q8_pair++) {
                        uint8_t column_idx_i = 0;
                        int sum_ys = 0;
                        int sum_xys = 0;

                        for (int i = 0; i < num_of_data_blocks_per_q8; i++) {
                            for (int k = 0; k < weights_per_u64_data_block; k ++) {
                                sum_ys += column[column_idx].qs[column_idx_i];
                                sum_xys += ((((uint64_t *) data_start)[0] >> (k * QK4_QBits)) & ((1 << 4) - 1)) * column[column_idx].qs[column_idx_i];
                        ...
                        sumf += (float) sum_ys * min_value + (float) sum_xys * mult_value;
                        column_idx += 1;
                    }
...
static const size_t GGML_TYPE_SIZE[GGML_TYPE_COUNT] = {
    ...
    // q4_x doesnt have a fixed "type" size
...
enum ggml_type ggml_ftype_to_ggml_type(enum ggml_ftype ftype) {
    ...
    switch (ftype) {
        ...
        case GGML_FTYPE_MOSTLY_Q4_X:          wtype = GGML_TYPE_Q4_X;  break;
...
static inline bool ggml_is_contiguous(const struct ggml_tensor * tensor) {
    static_assert(GGML_MAX_DIMS == 4, "GGML_MAX_DIMS is not 4 - update this function");
    GGML_ASSERT(tensor->type != GGML_TYPE_Q4_X);
...
struct ggml_tensor * ggml_new_tensor_impl(
    ...
    //GGML_ASSERT(type != GGML_TYPE_Q4_X); used here
...
static void ggml_compute_forward_dup_same_cont(
    ...
    GGML_ASSERT(src0->type != GGML_TYPE_Q4_X);
...
static void ggml_compute_forward_mul_mat_q_f32(
    ...
    // printf("MUL type: %d x %d = %d, op type: %d\n", src0->type, src1->type, dst->type, params->type);
    // printf("ith %d, nth %d\n", ith, nth);
    ...
    if (type == GGML_TYPE_Q4_X) {
        ...
        vec_dot_type = quantize_fns[type].vec_dot_type;
        quantize_row_q_dot = quantize_fns[type].quantize_row_q_dot;
    }
    ...
    // rows per thread
    const int rows_per_th = (n_rows + nth - 1)/nth;

    // row range for this thread
    const int ir0 = rows_per_th*ith;
    const int ir1 = MIN(ir0 + rows_per_th, n_rows);
...
#endif
                            {
                                enum ggml_type type_q;

                                if (node->src0->type == GGML_TYPE_Q4_X) {
                                    type_q = GGML_TYPE_Q8_0;
                                } else {
                                    type_q = quantize_fns[node->src0->type].vec_dot_type;
                                }
...
size_t ggml_quantize_q4_0(const float * src, void * dst, int n, int k, int64_t * hist, FILE* debug_fp) {
    ...
    //printf("ggml_quantize_q4_0: nb %d, n %d, k %d, qk4_0 %d, sizeof block: %zu\n", nb, n, k, QK4_0, sizeof(block_q4_0));
...
size_t ggml_quantize_q4_x(const ggml_fp16_t * src, void * dst_void, int n, int k, int64_t * hist, uint64_t * extra_data, uint32_t tensor_width, FILE* debug_fp) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        //printf("f16: %lx\nf16_count: %d\n", fp16s[0], fp16_count);
        uint64_t * fp16_data = (uint64_t *) (dst + dst_offset);
...
size_t ggml_quantize_chunk(enum ggml_type type, const ggml_fp16_t * src_raw, const float * src, void * dst, int start, int n, int64_t * hist, uint64_t * extra_data, uint32_t tensor_width, FILE* debug_fp) {
    ...
    //fprintf(stderr, "quantizing chunk, size: %d, storage size: %zu\n", n, result);
    return result;
}
